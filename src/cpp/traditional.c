...
static void replace_args_and_push (cpp_reader *, struct fun_macro *, int);
		      ...
		      pfile->out.cur = out_start;
                      if( ( ! pfile->state.in_directive ) 
                       && ( ! ( node->flags & NODE_BUILTIN ) ) 
                        ...
                        xcp = FXTRAN_get_xcp (1, len + 1);
                        xcp->macro = macro1;
                        xcp->c1 = pfile->out.cur - pfile->out.base;
                        xcp->c2 = pfile->out.cur - pfile->out.base +
                          node->ident.len;
                        xcp->s[0].c1 = -1;
                        xcp->s[0].c2 = sizeof (FXTRAN_xcp) + 
                          2 * sizeof (FXTRAN_xcp_item);
                        memcpy ((char*)xcp + xcp->s[0].c2, text, len);
                     
                        FXTRAN_push_xcp (xcp, pfile->data);
#endif                        
		  ...
		  if (_cpp_arguments_ok (pfile, m, fmacro.node, fmacro.argc))
		    {
			 ...
			 output, and push its replacement text.  */
		      pfile->out.cur = (pfile->out.base
					     + fmacro.offset);
		      CUR (context) = cur;
		      replace_args_and_push (pfile, &fmacro, nspace);
		      goto new_context;
		    }
		}
	  ...
	  if (cur - 1 == start_of_input_line
	      ...
	      && !pfile->state.in_directive)
	    {
		 ...
		 call _cpp_handle_directive.  */
	      ...
	      out = pfile->out.cur;
              if ((memcmp (cur, "include", 7) == 0) && pfile->noinclude)
                goto skip_directive;
...
/* Push a context holding the replacement text of the macro NODE on
   ...
   macro with no arguments.  */
static void
replace_args_and_push (cpp_reader *pfile, struct fun_macro *fmacro, int nspace)
{
  ...
  if (macro->paramc == 0)
      ...
      for (exp = macro->exp.text;;)
	{
...
#ifdef _FXTRAN_XCP_H
      {
        ...
        xcp->c1 = pfile->out.cur - pfile->out.base;
        xcp->c2 = xcp->c1 + 
          fmacro->node->ident.len + 2 + len0 - 1 + nspace;

      }		    
 
      st = sizeof (FXTRAN_xcp) + 2 * sizeof (FXTRAN_xcp_item) * sa;
      sa = 0;
#endif
      for (exp = macro->exp.text;;)
	{
...
#ifdef UNDEF
void FXTRAN_print_macro( cpp_hashnode* hnode ) {
